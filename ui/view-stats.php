	...
	<h2>
		<?php
			...
			if ( !in_array( 'add', $obj->actions_disabled ) && !in_array( 'add', $obj->actions_hidden ) ) {
				$link = pods_query_arg( array( 'action' . $obj->num => 'add', 'id' . $obj->num => '', 'do' . $obj->num = '' ), PodsUI::$allowed, $obj->exclusion() );
		...
		<?php
			...
			elseif ( !in_array( 'manage', $obj->actions_disabled ) && !in_array( 'manage', $obj->actions_hidden ) ) {
				$link = pods_query_arg( array( 'action' . $obj->num => 'manage', 'id' . $obj->num => '' ), PodsUI::$allowed, $obj->exclusion() );
					...
					<div class="submitbox" id="submitpost">
						<div id="major-publishing-actions">
							...
							<?php
								...
								if ( pods_is_admin( array( 'pods' ) ) && !in_array( 'regenerate_view', $obj->actions_disabled ) && !in_array( 'regenerate_view', $obj->actions_hidden ) ) {
							?>
								<div id="preview-action">
									<a class="preview button" href="<?php echo pods_query_arg( array( 'action' => 'regenerate_view' ) ) ?>"><?php _e( 'Regenerate View', 'pods-ajax-views' ); ?></a>
								</div>
								<!-- /#delete-action -->
							<?php
								}
												...
												?>
													<li>
														...
														<?php
																...
																$field_value = PodsForm::field_method( $tracking_field[ 'type' ], 'ui', $obj->id, $tracking_value, $tracking_field[ 'name' ], array_merge( $tracking_field, pods_var_raw( 'options', $tracking_field, array(), null, true ) ), $obj->fields[ 'view' ] );
